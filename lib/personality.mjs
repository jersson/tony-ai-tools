...
const SCHEMA = {
  ...
  tone: (v) => {
    if (typeof v !== 'string' || !v.trim()) {
      throw new TypeError('personality.tone must be a non-empty string');
    }
    return v.trim();
  },
  working_rule: (v) => {
    if (typeof v !== 'string' || !v.trim()) {
      throw new TypeError('personality.working_rule must be a non-empty string');
    }
    return v.trim();
  },
  updated_at: (v) => {
...
const REQUIRED = ['archetype', 'updated_at'];
