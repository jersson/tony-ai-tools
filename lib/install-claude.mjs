...
import { scaffoldTonyDir } from './install.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const PACKAGE_ROOT = resolve(__dirname, '..');
const PLUGIN_NAME = 'tony';
const PLUGIN_DESCRIPTION = 'Idea-driven product workflows — epics and user stories';
...
// or merges the tony entry into an existing marketplace.json. Returns the
...
// .claude/skills/tony) so skills are not registered twice.
...
export function installClaude(projectDir) {
  ...
  if (!added.ok && !/already/i.test(added.output)) {
    console.error(`Failed to register the tony marketplace:\n${added.output}`);
    process.exitCode = 1;
    return;
  }
  if (!added.ok) {
  ...
  if (!installed.ok) {
    console.error(`Failed to install the tony plugin:\n${installed.output}`);
    process.exitCode = 1;
    return;
  }
  ...
  // Project-level installs get the .tony/raw-documents drop point scaffolded.
  // Global installs are not tied to a project, so they are skipped.
  if (projectDir) {
    scaffoldTonyDir(projectDir);
  }

  console.log('Restart Claude Code, then use commands like: /tony:explore-idea');
}
