...
- If `<project_root>/.tony/personality.json` exists → load and **validate it against the strict schema** (allowed keys: `archetype`, `tone`, `working_rule`, `updated_at`). If it has an unmapped key, a missing required field, or an invalid archetype, **throw a processing error** naming the problem and stop — never proceed with an unparseable config. `node <package_root>/lib/personality.mjs`-style validation may be used, or validate by hand against the globals schema.
